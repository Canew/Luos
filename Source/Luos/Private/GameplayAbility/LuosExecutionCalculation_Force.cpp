...
ULuosExecutionCalculation_Force::ULuosExecutionCalculation_Force(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	static ConstructorHelpers::FClassFinder<UGameplayEffect> GE_STUN(TEXT("/Game/GameplayAbility/Effects/GE_Stun.GE_Stun_C"));
	if (GE_STUN.Succeeded())
	{
		StunEffectClass = GE_STUN.Class;
	}
}

void ULuosExecutionCalculation_Force::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	...
	float BaseForce = Spec.GetSetByCallerMagnitude(LuosGameplayTags::Data_Force, false, 0.0f);
	float StunDuration = Spec.GetSetByCallerMagnitude(LuosGameplayTags::Data_Duration, false, 0.0f);
	...
	if (ForceDone > 0.0f)
	{
		...
		{
			if (StunEffectClass && StunDuration > 0.0f)
			{
				FGameplayEffectSpecHandle EffectSpecHandle = SourceAbilitySystemComponent->MakeOutgoingSpec(StunEffectClass, 1.0f, SourceAbilitySystemComponent->MakeEffectContext());
				if (EffectSpecHandle.IsValid())
				{
					EffectSpecHandle.Data->SetDuration(StunDuration, true);
			...
			StunDuration = StunDuration == 0.0f ? KINDA_SMALL_NUMBER : StunDuration;
			if (ACharacter* HitCharacter = Cast<ACharacter>(HitActor))
			{
				if (UCharacterMovementComponent* CharacterMovementComponent = HitCharacter->GetCharacterMovement())
				{
					if (ULuosAnimInstance* LuosAnimInstance = Cast<ULuosAnimInstance>(HitCharacter->GetMesh()->GetAnimInstance()))
					{
						LuosAnimInstance->PushDisableRootMotion(TargetAbilitySystemComponent->GetCurrentMontage(), StunDuration);
					}
