...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LuosExecutionCalculation_Damage)

void ULuosExecutionCalculation_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	...
	float BaseDamage = Spec.GetSetByCallerMagnitude(LuosGameplayTags::Data_Damage, false, 0.0f);
	...
	if (HitActor && HitActor != EffectCauser)
	{
	...
	// If target's health is less or equal than zero, doesn't dealt damage.
	bool bTargetHealthFound = false;
	float TargetHealth = TargetAbilitySystemComponent->GetGameplayAttributeValue(ULuosAttributeSet::GetHealthAttribute(), bTargetHealthFound);
	if (bTargetHealthFound && TargetHealth > 0.0f && DamageDone > 0.0f)
	{
		// Apply a damage modifier, this gets turned into - health on the target
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(ULuosAttributeSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
